...
FSX_INSTALL_SNAPSHOT=${FSX_INSTALL_SNAPSHOT:-"false"}
FSX_INSTALL_SNAPSHOT_VERSION=${FSX_INSTALL_SNAPSHOT_VERSION:-"v6.2.1"}
...
if [[ "${HELM_CT_TEST}" == true ]]; then
  ...
  set -x
  set +e
  ${GINKGO_BIN} -p --nodes="${GINKGO_NODES}" -v "${TEST_PATH}" --timeout=5h -kubeconfig="${KUBECONFIG}" --junit-report=report.xml -gce-zone="${FIRST_ZONE}" -pr="${PULL_NUMBER}"
  TEST_PASSED=$?
  set -e
  set +x
